...
def calcPercent():
    global score, totalTries
    percentage = (score / 36)*100
    app.setLabel("percentage", "You achieved a score of {0:.2f}%".format(percentage))
    return
...
def check(button):
    ...
    if button == "Enter Answer":
        ...
        #closes questions window
        app.hideSubWindow("questions")
        #sets all variables to 0 so the user can restart
        x = 0
        y = 0
        score = 0
        tries = 0
...
app.addLabel("percentage", "")
app.setLabelBg("percentage", "light green")
